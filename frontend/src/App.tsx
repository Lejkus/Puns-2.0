...
function App() {
   ...
   const [gameResults, setGameResults] = useState<
      { id: string; nickname: string; score: number }[] | null
   >(null);
   ...
   useEffect(() => {
      if (socket) {
         ...
         socket.on(
            "gameEnded",
            (data: { players: { id: string; nickname: string; score: number }[] }) => {
   ...
   const handleJoinRoom = (roomId: string, password?: string) => {
      ...
      sessionStorage.setItem(
         ROOM_STORAGE_KEY,
         JSON.stringify({ roomId, password: password || "" })
      );
                                 ...
                                 <>
                                    ...
                                    {isGameStarted && (
                                       <div className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 mb-6 flex justify-between items-center shadow-lg">
                                          ...
                                          <div className="text-xl font-bold text-slate-300">
                                             Hasło:{" "}
                                             <span className="text-emerald-400 tracking-widest font-mono">
                                                {word ? word : "_ _ _ _ _"}
                                             </span>
                                          </div>
                                       </div>
                                    ...
                                    <div className="flex flex-col lg:flex-row gap-8 items-start justify-center w-full">
                                       ...
                                       <Chat
                                          ...
                                          disabled={
                                             isGameStarted && socket?.id === drawerId
                                          }
                                       />
                                    </div>
                                 </>
